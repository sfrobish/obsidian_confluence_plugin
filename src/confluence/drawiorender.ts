...
export async function renderAllDrawio(blocks: DiagramBlock[], logger: Logger): Promise<Array<RenderedDrawio | null>> {
	const results: Array<RenderedDrawio | null> = [];
	for (const b of blocks) {
		try {
			const svg = await renderOneDrawio(b.source);
			results.push({ block: b, svg });
		} catch (e) {
			const msg = e instanceof Error ? (e.stack ?? e.message) : String(e);
			logger.warn(`Draw.io rendering failed; falling back to a code block: ${b.filename}`, msg);
			results.push(null);
		}
	}
	return results;
...
async function renderOneDrawio(source: string): Promise<ArrayBuffer> {
