 ...
 */
export function resolveInstance(instances: ConfluenceInstance[], url: string): ConfluenceInstance | null {
 ...
 */
export function resolveTargetInstance(instances: ConfluenceInstance[], target: PublishTarget): ConfluenceInstance | null {
	const url = target.url.trim();
	if (url) return resolveInstance(instances, url);
	const parentUrl = target.parentUrl?.trim() ?? '';
	if (parentUrl) return resolveInstance(instances, parentUrl);
	const pageId = target.pageId.trim();
	if (pageId && instances.length === 1) return instances[0] ?? null;
	return null;
}

/** Effective URL used for target ownership and unmatched diagnostics. */
export function getRoutingUrl(target: PublishTarget): string {
	const url = target.url.trim();
	if (url) return url;
	const parentUrl = target.parentUrl?.trim() ?? '';
...
export function partitionTargets(
	...
	const owners = targets.map((target) => resolveTargetInstance(instances, target)?.id ?? null);
...
export function findTargetUrlForInstance(
	instances: ConfluenceInstance[],
	targets: PublishTarget[],
	instanceId: string,
): string | null {
	for (const target of targets) {
		const url = target.url.trim();
		if (url && resolveTargetInstance(instances, target)?.id === instanceId) return url;
	}
	return null;
}
...
export function groupByInstance(
	...
	for (const file of files) {
		const targets = getRoutingTargetsForFile(app, file, frontmatterKey);
		const routableTargets = targets.filter((target) => {
			const route = getRoutingUrl(target);
		...
		for (const target of routableTargets) {
			const inst = resolveTargetInstance(instances, target);
...
function getRoutingTargetsForFile(app: App, file: TFile, frontmatterKey: string): PublishTarget[] {
